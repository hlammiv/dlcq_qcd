...
def gather() -> dict:
    ...
    n7_path = ROOT / "data" / "paper" / "bar_ext_improved_N7.csv"
    if n7_path.exists():
        for key, ser in _read_house_csv(n7_path, "improved").items():
            bar_ext.setdefault(key, {}).update(ser)
    # N=8: our MXTRM-raised extension (2K=24-32) merged with the companion
    # scan's low-K points (2K<=22, read-only) — same Hamiltonian and
    # truncation convention, one grid.
    bar_n8 = _read_house_csv(ROOT / "data" / "paper" /
                             "bar_ext_improved_N8.csv", "improved")
    try:
        low = _read_house_csv(ROOT / "data" / "bar_scan" /
                              "bar_improved_N8.csv", "improved")
        for key, ser in low.items():
            bar_n8.setdefault(key, {}).update(
                {k: v for k, v in ser.items()
                 if k not in bar_n8.get(key, {})})
    ...
    for Nc in (5, 6, 7, 8):
        ...
        src = bar_n8 if Nc == 8 else bar_ext
        bser = src.get((Nc, 1, mg_ratio), {})
    ...
    if devs:
        ...
        numbers["ratio_worst_dev_pct"] = {
            ...
            "inputs": ["data/paper/bar_ext_improved_N567.csv",
                       "data/paper/bar_ext_improved_N8.csv",
                       "data/bar_scan/bar_improved_N8.csv"]
    ...
    # GMOR law, regenerated live from the peer-owned tool as a subprocess
    # (read-only on data/gmor_scan).  Parses the "does the large-N residual
    # vanish" linear fit: B +- err and slope A.
    import re as _re
    import subprocess as _sp
    gr = _sp.run([_s.executable if hasattr(_s, "executable") else "python3",
                  str(ROOT / "tools" / "gmor_chiral_limit.py"),
                  "--data-dir", str(ROOT / "data" / "gmor_scan"),
    ...
    mline = _re.search(
        ...
        r"A=([+-][\d.]+)", gr.stdout)
    if not mline:
        raise SystemExit("gmor: linear-law line not found in tool output")
    numbers["gmor_intercept"] = {
        "value": f"{float(mline.group(1)):+.4f} \\pm "
                 f"{float(mline.group(2)):.4f}",
        "hamiltonian": "improved", "fit_basis": "1/K",
        "inputs": [f"data/gmor_scan/improved_N{n}.csv"
                   for n in range(2, 9)],
        "note": "intercept of (M^2/GMOR - 1) = B + A(m/g), regenerated by "
                "tools/gmor_chiral_limit.py"}
    numbers["gmor_slope"] = {
        "value": f"{float(mline.group(3)):.2f}",
        "hamiltonian": "improved", "fit_basis": "1/K",
        "inputs": numbers["gmor_intercept"]["inputs"],
        "note": "slope A of the same law"}
