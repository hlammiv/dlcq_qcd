...
\caption{Inside the two candidates --- the $N=2$ tetraquark level and
the $N=4$ $B=2$ ground state --- at $m/g = 0.1$, $2K = 48$, standard
Hamiltonian, dominant Fock sectors ($\mathrm{LPN}$ as in
Fig.~\ref{fig:exotics}).  Left: single-particle structure functions.
Right: the two-parton momentum correlation $C(x_1, x_2)$.  The
tetraquark candidate's pairs concentrate along a ridge of fixed pair sum
near one half --- the momentum of a single meson --- while the $B=2$
state shows no two-cluster ridge.}
\label{fig:exoticwf}
\end{figure}
...
verdict splits the two channels.  The tetraquark candidate's correlation
ridges at a pair sum near one half, the momentum of one meson: it is a
bound two-meson molecule, not a compact four-quark state.  The $B=2$
ground state shows no such ridge; its pairs pack democratically at small
$x$, consistent with a compact six- or eight-parton object rather than a
deuteron-like molecule --- though a sharper statement needs color-singlet
