...
\begin{figure}
\includegraphics[width=\columnwidth]{figs/fig4_higher_fock.pdf}
\caption{Higher-Fock structure functions, $N=3$, ground states, standard
Hamiltonian, $m/g = 1.6$ (solid, with the 1990 multipliers) and $0.1$
(dashed): the meson four-quark sector and the baryon five-parton sector's
quark and antiquark content.  Top: $2K = 14$ and $15$; rings are the
hand-measured values of the published panels, which trace the $m/g=1.6$
series.  Bottom: $2K = 100$ ($\mathrm{LPN}=6$) and $99$
($\mathrm{LPN}=5$).}
\label{fig:higherfock}
\end{figure}

The higher-Fock sectors themselves are Fig.~\ref{fig:higherfock}.  The
right panels carry the physics the 1990 paper flagged as ``an indication
of the pion content of the baryon'': the five-parton sector's antiquark
distribution.  At $2K = 99$ its shape is unambiguous.  The antiquark sits
at small $x$, carrying a few percent of the momentum fraction its quarks
do, and strong coupling pushes it further toward the endpoint --- a wee
partonic cloud, exactly where a pion dressing should live.  The
hand-measured 1990 points land on our curves in both panels.  The
published $m/g = 0.1$ curves of this figure are not overlaid, for a
documented reason rather than a deferred one: no round multiplier
reconciles them with any series our solvers produce, their legend cannot
be read with confidence, and the shape disagreement survives every frame
recalibration --- the same era of hand conversion that produced the
five-quark defect of Fig.~\ref{fig:baryonwf}.
...
Ref.~\cite{Hornbostel:1988fb}; in the last panel the multiplier sits on
the valence and the four-quark curve is unscaled, the convention of the
original's dual-axis panel.  Bottom: $2K = 100$, $\mathrm{LPN} = 6$, lowest 40
...
recovered independently.  At $2K = 100$ it lands on level 10, and the
...
Ref.~\cite{Hornbostel:1988fb}.  Bottom: $2K = 99$ and $50$ at
